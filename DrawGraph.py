...
class DrawElements():
    ...
    def DrawCylinder(self,Element,Type):
        ...
        X, Y, Z = [Element.SupportVector[i] + a[i] * t + r * np.sin(theta) * n1[i] + r * np.cos(theta) * n2[i] for i in [0, 1, 2]]
    ...
    def DrawCircle(self,Element,Type):
        ...
        theta = np.linspace(0, 2 * np.pi, 10)
        X, Y, Z = [Element.SupportVector[i] +  r * np.sin(theta) * n1[i] + r * np.cos(theta) * n2[i] for i in [0, 1, 2]]
        verts = [zip(X,Y,Z)]


        Collection = Poly3DCollection(verts,linewidths=1,alpha = Type.alpha)
        FaceColor = Type.color #self.colors[Type]
        Collection.set_facecolor(FaceColor)
        Collection.set_edgecolor("black")
        self.ax.add_collection3d(Collection)

    def DrawElementsPoly(self):
        '''Draw Stuff in 3d,using polys'''
        self.ax.cla()
        i=0
        for el in self.OpticalElements:
            ...
            if isinstance(el.Geometry, Geometries.Triangle):
                ...
                x = [el.Geometry.TrianglePoint1[0],el.Geometry.TrianglePoint2[0],el.Geometry.TrianglePoint3[0]]
                y = [el.Geometry.TrianglePoint1[1],el.Geometry.TrianglePoint2[1],el.Geometry.TrianglePoint3[1]]
                z = [el.Geometry.TrianglePoint1[2],el.Geometry.TrianglePoint2[2],el.Geometry.TrianglePoint3[2]]
                verts = [zip(x,y,z)]
                Collection = Poly3DCollection(verts,linewidths=1,alpha = el.Type.alpha)
                FaceColor = el.Type.color
                Collection.set_facecolor(FaceColor)
    ...
    def DrawLine(self,point1,point2,LineColor):
        ...
        self.ax.plot([point1[0],point2[0]] , [point1[1],point2[1]] , [point1[2],point2[2]], color = LineColor)
